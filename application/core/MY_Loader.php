...
class MY_Loader extends MX_Loader {
	...
	private $_javascript_page = array();
	private $_css = array();
	private $_css_page = array();
	...
	function css_page(){
		$css_page_files = func_get_args();

		foreach($css_page_files as $css_file){
			$css_file = substr($css_file,0,1) == '/' ? substr($css_file,1) : $css_file;
			...
			if(!in_array($css_file, $this->_css_page))
				$this->_css_page[] = $css_file;
		}
		return;
	}
	...
	function js_page(){
		...
		foreach($script_files as $script_file){
			...
			if(!in_array($script_file, $this->_javascript_page))
				$this->_javascript_page[] = $script_file ;
	...
	function get_css_page_files(){
		return $this->_css_page;
	}
	...
	function get_js_page_files(){
		return $this->_javascript_page;
	}
